<div class="card">
  ...
  <div class="card-body">
    ...
    <div>
      <a class="btn btn-primary btn-flat" href="?c=employee&a=FormCrear">
    ...
    <div class="table-responsive">
      <table id="tabla-activos" class="table table-bordered table-striped">
        ...
        ?>
        <tr id="fila-<?=$r->id_employee?>-activos" data-id="<?=$r->id_employee?>">
          <td><?=htmlspecialchars($r->name_employee ?? '')?></td>
          <td><?=htmlspecialchars($r->lastname_employee ?? '')?></td>
          <td><?=htmlspecialchars($r->address_employee ?? '')?></td>
          <td><?=htmlspecialchars($r->name_type_doc ?? 'Sin tipo')?></td>
          <td><?=htmlspecialchars($r->doc_num ?? '')?></td>
          <td><?=htmlspecialchars($r->path_img_employee ?? '')?></td>
          <td><?=htmlspecialchars($r->name_role_employee ?? '')?></td>
          ...
          <td>
            <a href="?c=employee&a=FormEditar&id=<?=$r->id_employee?>" 
          ...
          <div class="card-body">
            <div class="table-responsive">
              <table id="tabla-inactivos" class="table table-bordered table-striped">
                ...
                ?>
                <tr id="fila-<?=$r->id_employee?>-inactivos" class="registro-inactivo" data-id="<?=$r->id_employee?>">
                  <td><?=htmlspecialchars($r->name_employee ?? '')?></td>
                  <td><?=htmlspecialchars($r->lastname_employee ?? '')?></td>
                  <td><?=htmlspecialchars($r->address_employee ?? '')?></td>
                  <td><?=htmlspecialchars($r->name_type_doc ?? 'Sin tipo')?></td>
